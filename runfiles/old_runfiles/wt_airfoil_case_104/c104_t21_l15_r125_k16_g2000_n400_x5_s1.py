...
pop = newGeneration(airfoil_fitness, pop, normalizationVector = [1]*N_k, encodingTypes=[float]*N_k, lowerBounds=[-2.0]*N_k, upperBounds=[2.0]*N_k, tau=tau, initalize=True, comm=comm, CL = CL, rLD = rLD, re = re, file_system = file_system, N_retries = N_retries)
pop = comm.bcast(pop, root=0)
...
for i in range(1,N_generations+1):
    ...
    pop = newGeneration(airfoil_fitness, pop, normalizationVector = [1]*N_k, encodingTypes=[float]*N_k, lowerBounds=[-2.0]*N_k, upperBounds=[2.0]*N_k, tau=tau, initalize=False, comm=comm, CL = CL, rLD = rLD, re = re, file_system = file_system, N_retries = N_retries)
    pop = comm.bcast(pop, root=0)
    if rank == 0:
        if len(pop) == 0:
            pop = pop_cache
