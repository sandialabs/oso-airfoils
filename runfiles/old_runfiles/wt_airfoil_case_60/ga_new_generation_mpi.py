...
def newMember(N_k,tau):
    ...
    for i in range(0,100):
        K = [random.uniform(-0.1,0.8) for j in range(0,int(N_k/2))]+[random.uniform(-0.8,0.1) for j in range(0,int(N_k/2))]
        ...
        if tau_loc_u >= 0.20 and tau_loc_l >= 0.20 and not any([math.isnan(rpv) for rpv in K_candidate]):
            break
    ...
    if rank == 0:
        ...
        for i in range(0,len(resultantPop)):
            ...
            try:
                ...
                # thickness scaling threw a nan, probably not a viable airfoil
                N_k = len(resultantPop[i])
                resultantPop[i] = newMember(N_k,tau)

            if tau_loc_u < 0.20 or tau_loc_l < 0.20:
                N_k = len(resultantPop[i])
                resultantPop[i] = newMember(N_k,tau)
            else:
