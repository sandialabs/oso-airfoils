...
def core_fitness_function(x):
    ...
    try:
        ...
        clean_moment_constraint_values = {
            '21' : -0.183, 
            '24' : -0.161,
            '27' : -0.158,
            '30' : -0.148,
        ...
        rough_moment_constraint_values = {
            '21' : -0.157,
            '24' : -0.150,
            '27' : -0.145,
            '30' : -0.138,
        ...
        if CMc_in is not None:
            if CMc_in is True:
                cm_design_clean = clean_moment_constraint_values[str(int(100*tau))]
            else:
                asset(isinstance(CMc_in,float))
                cm_design_clean = CMc_in

        if CMr_in is not None:
            if CMr_in is True:
                cm_design_rough = rough_moment_constraint_values[str(int(100*tau))]
            else:
                asset(isinstance(CMr_in,float))
                cm_design_rough = CMr_in
        ...
        # Minimizing, so multiply by -1 to maximize LoD
        # obj += clean_LoD_weighting*-1*(LoD_clean_at_design_alpha-LoD_reward_threshold_clean)
        # obj += rough_LoD_weighting*-1*(LoD_rough_at_design_alpha-LoD_reward_threshold_rough)

        # obj += clean_LoD_weighting*-1*(LoD_clean_at_design_alpha-190)**3
        # obj += rough_LoD_weighting*-1*(LoD_rough_at_design_alpha-118)**3

        obj += clean_LoD_weighting*-1*(LoD_clean_at_design_alpha-LoD_reward_threshold_clean)

        # if LoD_rough_at_design_alpha > LoD_reward_threshold_max_rough :
        #     obj += rough_LoD_weighting*-1*(LoD_reward_threshold_max_rough-LoD_reward_threshold_rough)**3
        #     obj += rough_LoD_weighting*-1*(LoD_rough_at_design_alpha-LoD_reward_threshold_rough)
        # else:
        #     obj += rough_LoD_weighting*-1*(LoD_rough_at_design_alpha-LoD_reward_threshold_rough)**3
        ...
        # ----------------------
        # LoD_rough_1degree_left = np.interp(0.85*alpha_design, 
        LoD_rough_1degree_left = np.interp(alpha_design - alpha_falloff_offset, 
                                             np.array(alpha_rough)[0:positive_peak_index_rough], 
                                             np.array(LoD_rough)[0:positive_peak_index_rough] )

        # LoD_rough_1degree_right = np.interp(1.15*alpha_design, 
        ...
        # # Straight reject if further forward than 20%
        # if tau_loc_u < 0.20:
        #     return [pid, np.inf, False, -40] + [0]*N_reported + [0]*N_constraints
        ...
        # ----------------------
        if CMc_in is not None:
            alpha_design_range_indicies = [i for i,alpha_val in enumerate(alpha_clean) if alpha_val >= alpha_design-5 and alpha_val <= alpha_design+5]
            min_cm_clean = min([cm_clean[i] for i in alpha_design_range_indicies])
            if min_cm_clean < cm_design_clean:
                obj += clean_moment_weighting * abs(cm_design_clean - min_cm_clean)
        ...
        # ----------------------
        if CMr_in is not None:
            alpha_design_range_indicies = [i for i,alpha_val in enumerate(alpha_rough) if alpha_val >= alpha_design-5 and alpha_val <= alpha_design+5]
            min_cm_rough = min([cm_rough[i] for i in alpha_design_range_indicies])
            if min_cm_rough < cm_design_rough:
                obj += rough_moment_weighting * abs(cm_design_rough - min_cm_rough)
...
if __name__ == '__main__':
    ...
    for i,ind in enumerate(airfoil_seeds):
        ...
        afl = Kulfan()
        afl.upperCoefficients = K_upper
        afl.lowerCoefficients = K_lower
        t = round(afl.tau*100)

        afl_geo = Kulfan(TE_gap = te_gap_lookup[str(int(t))])
        afl_geo.upperCoefficients = K_upper
        afl_geo.lowerCoefficients = K_lower
        afl_geo.changeOrder(4)
        # plt.figure()
        # plt.plot(afl_geo.xcoordinates, afl_geo.ycoordinates,lw=0.2)
        # plt.axis('equal')
        # plt.savefig('airfoil_%d.png'%(i),dpi=300)
        # plt.close()
