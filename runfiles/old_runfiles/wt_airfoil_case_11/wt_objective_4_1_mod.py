...
def airfoil_fitness(x):
    try:
        ...
        obj += 20.0*800/(500+LoD_rough_at_design_cl)
        # equal weigting, baseline objective
        
        obj += 0.5*800/(500+max(LoD_clean))
        obj += 2.0*800/(500+max(LoD_rough))
        ...
        # ----------------------
        # attempt to flatten the upper aft section
        # ----------------------
        bsl_cutoff = 0.45
        zeta_05 =np.interp(bsl_cutoff, afl.psi, afl.zetaUpper) 
        er_tot = 0
        for i,psv in enumerate(afl.psi):
            if psv >= bsl_cutoff:
                er = afl.zetaUpper[i] - np.interp(psv, [bsl_cutoff, 1.0], [zeta_05, afl.zetaUpper[-1]])
                er_tot += abs(er)
        obj += er_tot*1e3
