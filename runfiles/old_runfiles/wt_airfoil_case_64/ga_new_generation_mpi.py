    ...
    if rank == 0:
        ...
        for i in range(0,len(resultantPop)):
            ...
            if any([math.isnan(kv) for kv in K]):
                N_k = len(K)
                K = newMember(int(N_k/2),tau,1)#[random.uniform(0.1,0.8)] + [random.uniform(-0.1,0.8) for j in range(0,int(N_k/2)-1)]   +   [random.uniform(-0.8,-0.1)] + [random.uniform(-0.8,0.4) for j in range(0,int(N_k/2)-1)]  #newMember(N_k,tau)
            Ku = K[0:int(len(K)/2)]
            Kl = K[int(len(K)/2):]
            ...
            try:
                ...
                if any([math.isnan(rpv) for rpv in resultantPop[i]]):
                    ...
                    resultantPop[i] = newMember(int(N_k/2),tau,1)#[random.uniform(0.1,0.8)] + [random.uniform(-0.1,0.8) for j in range(0,int(N_k/2)-1)]   +   [random.uniform(-0.8,-0.1)] + [random.uniform(-0.8,0.4) for j in range(0,int(N_k/2)-1)]  #newMember(N_k,tau)
            except:
                N_k = len(K)
                resultantPop[i] = newMember(int(N_k/2),tau,1)#[random.uniform(0.1,0.8)] + [random.uniform(-0.1,0.8) for j in range(0,int(N_k/2)-1)]   +   [random.uniform(-0.8,-0.1)] + [random.uniform(-0.8,0.4) for j in range(0,int(N_k/2)-1)]  #newMember(N_k,tau)    
