...
def newGeneration(  fitnessFunction, 
    ...
    if rank == 0:
        ...
        for i in range(0,len(resultantPop)):
            ...
            # if resultantPop[i][-1] < 0.0 or math.isnan(resultantPop[i][-1]):
            #     resultantPop[i][-1] = random.uniform(0.002,0.04)
            # afl = Kulfan(TE_gap = resultantPop[i][-1])
            K = resultantPop[i]
            if any([math.isnan(kv) for kv in K]) or K[-1]<=0:
                N_k = len(K)-1
            ...
            try:
                ...
                resultantPop[i] = afl.upperCoefficients.magnitude.tolist() + afl.lowerCoefficients.magnitude.tolist() + [afl.constants.TE_gap]
                if any([math.isnan(rpv) for rpv in resultantPop[i]]):
                    N_k = len(resultantPop[i])-1
                ...
                if len(resultantPop[i]) != len(resultantPop[i-1]):
                    raise ValueError('Error in newMember')
            except:
                N_k = len(resultantPop[i])-1
                resultantPop[i] = newMember(int(N_k/2),tau,1)[0]#[random.uniform(0.1,0.8)] + [random.uniform(-0.1,0.8) for j in range(0,int(N_k/2)-1)]   +   [random.uniform(-0.8,-0.1)] + [random.uniform(-0.8,0.4) for j in range(0,int(N_k/2)-1)]  #newMember(N_k,tau)    
                if len(resultantPop[i]) != len(resultantPop[i-1]):
    ...
    if rank == 0:
        ...
        # except:
        #     with open('debug_nsga.txt','w') as f:
        #         for rp in resultantPop:
        #             f.write( str(rp) + '\n')
        #         f.write('----\n')
        #         for res in result:
        #             f.write( str(res) + '\n')
        #     raise ValueError('Error appending fitness to resultantPop')
        for ii in range(2,len(result[0])):
            da = np.array([result[i][ii] for i in range(0,len(result))]).astype(float)
            data = np.append(data,np.array([da]).T,axis=1)
        ...
        for i in range(0,remaining_slots):
            finalPop.append( last_front_inds[i][0:-2] )
