...
for fc in filecodes:
    ...
    if 'm' in fc:
        if len(fc) < 2:
            ...
            CMc_min = float(fc[1:])/(-1000.0)
    if 'p' in fc:
        if len(fc) < 2:
            CMr_min = True
        else:
            CMr_min = float(fc[1:])/(-1000.0)
...
if rank == 0:
    ...
    for i,p in enumerate(pop):
        afl = Kulfan(TE_gap = random.uniform(0.002,0.04))
        K = p
        Ku = K[0:int(len(K)/2)]
        Kl = K[int(len(K)/2):]
        ...
        pop[i] = afl.upperCoefficients.magnitude.tolist() + afl.lowerCoefficients.magnitude.tolist() + [afl.constants.TE_gap]
...
pop = newGeneration(airfoil_fitness, pop, normalizationVector = [1]*(N_k+1), encodingTypes=[float]*(N_k+1), lowerBounds=[-2.0]*(N_k+1), upperBounds=[2.0]*(N_k+1), tau=tau, initalize=True, comm=comm, CL = CL, re = re, file_system = file_system, N_retries = N_retries, CMc_min = CMc_min, CMr_min = CMr_min)
pop = comm.bcast(pop, root=0)
...
for i in range(1,N_generations+1):
    ...
    pop = newGeneration(airfoil_fitness, pop, normalizationVector = [1]*(N_k+1), encodingTypes=[float]*(N_k+1), lowerBounds=[-2.0]*(N_k+1), upperBounds=[2.0]*(N_k+1), tau=tau, initalize=False, comm=comm, CL = CL, re = re, file_system = file_system, N_retries = N_retries, CMc_min = CMc_min, CMr_min = CMr_min)
    pop = comm.bcast(pop, root=0)
    if rank == 0:
        if len(pop) == 0:
            pop = pop_cache
