...
# mpirun -n 8 python -m mpi4py c53_t24_k8_n200.py
...
if rank == 0:
    ...
    if 'karch' in str(path_to_here):
        ...
        comp_afls = comparisonAirfoils[str(int(100*tau))]
        ...
        if len(pop) >= 1:
            ...
            plt.title('Thickness: 0.%s , Generation: %d, Objective: %.5f'%(str(int(100*tau)),0,rs[0]))
            plt.ylim([-0.25,0.25])
            ax.set_aspect('equal',adjustable='box')
            plt.tight_layout()
            plt.legend(loc='upper right', fontsize="10")
            plt.savefig(ldr + folderstr + '/Generation_%d_airfoils.png'%(0))
            plt.close()
    ...
    if rank == 0:
        if 'karch' in str(path_to_here):
            ...
            if len(pop) >= 1:
                ...
                plt.title('Thickness: 0.%s , Generation: %d, Objective: %.5f'%(str(int(100*tau)),i,rs[0]))
                plt.ylim([-0.25,0.25])
                ax.set_aspect('equal',adjustable='box')
                plt.tight_layout()
                plt.legend(loc='upper right', fontsize="10")
                plt.savefig(ldr + folderstr + '/Generation_%d_airfoils.png'%(i))
                plt.close()
