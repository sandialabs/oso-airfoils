    ...
    try:
        ...
        for i in range(0,len(K_upper)):
            if K_upper[i] <= K_lower[i]:
                return [pid, np.inf, False, -10] + [0]*15 + [0]*10
        
        # ----------------------
        # reject too large coefficients
        # ----------------------
        if max(abs(np.array(K_upper)))>2.0:
            return [pid, np.inf, False, -20] + [0]*15 + [0]*10
        if max(abs(np.array(K_lower)))>2.0:
            return [pid, np.inf, False, -30] + [0]*15 + [0]*10
        ...
        try:
            ...
            return [pid, np.inf, False, -70] + [0]*15 + [0]*10
        ...
        if cl_clean[positive_peak_index_clean] > cl_design:
            ...
            return [pid, np.inf, False, -80] + [0]*15 + [0]*10
        ...
        obj += 4.0*800/(500+LoD_clean_at_design_cl)
        obj += 4.0*800/(500+LoD_rough_at_design_cl)
        # equal weigting, baseline objective
        
        obj += 0.5*800/(500+max(LoD_clean))
        obj += 0.5*800/(500+max(LoD_rough))
        ...
        return [pid, np.inf, False, -90] + [0]*15 + [0]*10
