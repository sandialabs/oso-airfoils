...
def tryFastrun(K_upper, K_lower, cl_design, Re, N_crit, xtr_u, xtr_l):
    # try fastrun
    fastrunWorks = False
    ...
    if cl_design < max(cl_quick):
        ...
        if cl_quick[slice_idx] == cl_quick[slice_idx-1]:
            ...
            alpha_design_guess = alpha_quick[slice_idx-1] + (alpha_quick[slice_idx] - alpha_quick[slice_idx-1])*dist
        res_aguess = run_xfoil('alfa', K_upper, K_lower, alpha_design_guess, Re=Re, N_crit=N_crit, xtr_u=xtr_u, xtr_l=xtr_l)
        if abs(res_aguess['cl'] - cl_design)/cl_design <= 0.05:
            ...
            res_075aguess = run_xfoil('alfa', K_upper, K_lower, 0.75*alpha_design_guess, Re=Re, N_crit=N_crit, xtr_u=xtr_u, xtr_l=xtr_l)
            ...
            for iii in range(0,100):
                res_stall_search = run_xfoil('alfa', K_upper, K_lower, alpha_start_search+(iii+1)*0.5, Re=Re, N_crit=N_crit, xtr_u=xtr_u, xtr_l=xtr_l)
    ...
    try:
        ...
        for i in range(0,len(K_upper)):
            if K_upper[i] <= K_lower[i]:
                return [pid, np.inf, False, -10] + [0]*15 + [0]*7
        
        # ----------------------
        # reject too large coefficients
        # ----------------------
        if max(abs(np.array(K_upper)))>2.0:
            return [pid, np.inf, False, -20] + [0]*15 + [0]*7
        if max(abs(np.array(K_lower)))>2.0:
            return [pid, np.inf, False, -30] + [0]*15 + [0]*7
        ...
        if leruViolation != 0:
            return [pid, np.inf, False, -40] + [0]*15 + [0]*7
        if lerlViolation != 0:
            return [pid, np.inf, False, -50] + [0]*15 + [0]*7
        ...
        # plt.plot(afl_geo.xcoordinates, afl_geo.ycoordinates)
        # plt.plot([te_frac, 1.0, te_frac], [upper_10deg_cone,0,lower_10deg_cone])
        # plt.xlim([0.97,1.0])
        ...
        for i,psi_val in enumerate(afl_geo.psi):
            if psi_val >= te_frac:
                ...
                if zeta_upper_val < h_upper or zeta_lower_val > h_lower:
                    return [pid, 1e10, False, -60] + [0]*15 + [0]*7
        ...
        try:
            clean_results = tryFastrun(K_upper, K_lower, cl_design, Re, N_crit=9.0, xtr_u=1.0, xtr_l=1.0)
            ...
            for alpha in alphaList:
                res = run_xfoil('alfa', K_upper, K_lower, alpha, Re=Re, N_crit=9.0, xtr_u=1.0, xtr_l=1.0)
                if res is not None:
        ...
        try:
            if fastrun_clean_success:
                rough_results = tryFastrun(K_upper, K_lower, cl_design, Re, N_crit=3.0, xtr_u=0.05, xtr_l=0.05)
            else:
            ...
            rough_results=[]
            for alpha in alphaList:
                res = run_xfoil('alfa', K_upper, K_lower, alpha, Re=Re, N_crit=3.0, xtr_u=0.05, xtr_l=0.05)
                if res is not None:
        ...
        try:
            ...
            return [pid, np.inf, False, -70] + [0]*15 + [0]*7
        ...
        if cl_clean[positive_peak_index_clean] > cl_design:
            ...
            return [pid, np.inf, False, -80] + [0]*15 + [0]*7
        ...
        obj += 1.5*800/(500+LoD_clean_at_design_cl)
        obj += 1.5*800/(500+LoD_rough_at_design_cl)
        ...
        obj += (max([0.15, abs(percent_change_LoD_clean_left )])-0.15) * 1e2
        obj += (max([0.15, abs(percent_change_LoD_clean_right)])-0.15) * 1e2
        ...
        # penalize if greater than 15%
        # assert(percent_delta_LoD_clean_to_rough_at_alpha_design >=0)
        obj += (max([0.15, abs(percent_change_LoD_rough_left )])-0.15) * 1e2
        obj += (max([0.15, abs(percent_change_LoD_rough_right)])-0.15) * 1e2
        ...
        # ----------------------
        # Verify thickness, though this should be true by construction
        # ----------------------
        ...
        return [pid, np.inf, False, -90] + [0]*15 + [0]*7



# afl = generateAirfoil('du_95-w-180')
# x = {}
# x['individual'] = afl.upperCoefficients.magnitude.tolist() + afl.lowerCoefficients.magnitude.tolist()
# # x['individual'] = [0.2,0.2,-0.2,-0.2]
# x['tau'] = 0.18
