...
filteredFiles = []
for fl in files:
    # Sort, looking only for the mhkf1 files right now
    if fl[0]!='.' and fl[0]!='_' and ('.png' in fl) and ('mhkf1' in fl):
...
for fl in filteredFiles:
    ...
    # find the point where miny and maxy are closest to each other; this is the leading edge
    cross = np.argmin(np.abs(np.array(min_y_arr) - np.array(max_y_arr)))
    # print(cross)
    zeroRow = (min_y_arr[cross] + max_y_arr[cross])/2.0

    # Find where the pixel turns white again along zeroRow starting from minColumn
    zeroRow_int = int(round(zeroRow))
    leading_edge_end = minColumn
    for j in range(minColumn, maxColumn):
        quad = img[zeroRow_int][j]
        if quad[0] == 1 and quad[1] == 1 and quad[2] == 1:
            leading_edge_end = j
            break
    zeroColumn = (minColumn + leading_edge_end) / 2.0

    plt.plot([zeroColumn, maxColumn],[zeroRow, zeroRow],'b' )
    plt.plot([cross, cross],[0, len(img)],'g')
    ...
    # Automatically compute rotation angle from trailing edge midpoint relative to leading edge
    te_mid_y = (min_y_arr[maxColumn] + max_y_arr[maxColumn]) / 2.0
    y_te = -1 * (te_mid_y - zeroRow) / (maxColumn - zeroColumn)
    angle = np.arctan2(y_te, 1.0)
    x_le = 0
    y_le = 0
    x_rot = (afl.xcoordinates - x_le) * np.cos(angle) - (afl.ycoordinates - y_le) * np.sin(angle) + x_le
    y_rot = (afl.xcoordinates - x_le) * np.sin(angle) + (afl.ycoordinates - y_le) * np.cos(angle) + y_le
    
    plt.plot(x_rot*(maxColumn-zeroColumn)+zeroColumn,-1*y_rot*(maxColumn-zeroColumn)+zeroRow,'k')
