    ...
    try:
        ...
        for i in range(0,len(K_upper)):
            if K_upper[i] <= K_lower[i]:
                return [pid, np.inf, False, -10] + [0]*15 + [0]*8
        
        # ----------------------
        # reject too large coefficients
        # ----------------------
        if max(abs(np.array(K_upper)))>2.0:
            return [pid, np.inf, False, -20] + [0]*15 + [0]*8
        if max(abs(np.array(K_lower)))>2.0:
            return [pid, np.inf, False, -30] + [0]*15 + [0]*8
        ...
        if leruViolation != 0:
            return [pid, np.inf, False, -40] + [0]*15 + [0]*8
        if lerlViolation != 0:
            return [pid, np.inf, False, -50] + [0]*15 + [0]*8
        ...
        for i,psi_val in enumerate(afl_geo.psi):
            if psi_val >= te_frac:
                ...
                if zeta_upper_val < h_upper or zeta_lower_val > h_lower:
                    return [pid, 1e10, False, -60] + [0]*15 + [0]*8
        ...
        try:
            ...
            return [pid, np.inf, False, -70] + [0]*15 + [0]*8
        ...
        if cl_clean[positive_peak_index_clean] > cl_design:
            ...
            return [pid, np.inf, False, -80] + [0]*15 + [0]*8
