...
def polarPlot(dataList, airfoil_coords=None, legend_entries=None,
              ...
              width_ratios=None, wspace=0.12, show_cpmin=True,
              cl_design=None, legend_ncols=None):
    """Plot polar data.
    ...
    width_ratios : list of floats, optional
        Relative widths of the polar panels. When show_cpmin=True (default), 5 values
        [cpmin, drag, CL/CM, L/D, xtr]; when show_cpmin=False, 4 values [drag, CL/CM, L/D, xtr].
    wspace : float
        Horizontal spacing between the polar panels.
    show_cpmin : bool, optional
        If False, the Cp_min panel is omitted and the layout uses 4 columns.
        The airfoil geometry panel spans the right 5/8 of the top row.
    """
    ...
    gs_width_ratios = [w / sum(width_ratios) * n_cols for w in width_ratios]
    ...
    if show_top_row:
        if airfoil_coords is not None:
            ...
            # Airfoil panel spans right portion of top row; estimate physical width in inches
            # (0.80 accounts for wspace, constrained-layout margins, etc.)
            if show_cpmin:
                airfoil_col_frac = sum(width_ratios[2:]) / sum(width_ratios)
            else:
                airfoil_col_frac = 5.0 / 8.0  # top row split [3, 5]
        ...
        if show_cpmin:
            ...
            top_gs     = GridSpecFromSubplotSpec(1, 2, subplot_spec=outer_gs[0], width_ratios=[3, 5], wspace=wspace)
            ax_legend  = fig.add_subplot(top_gs[0, 0])
            ax_airfoil = fig.add_subplot(top_gs[0, 1])
        bot_gs = GridSpecFromSubplotSpec(1, n_cols, subplot_spec=outer_gs[1],
                                         width_ratios=gs_width_ratios, wspace=wspace)
    else:
        fig = plt.figure(figsize=(fig_width, bot_row_height_in), dpi=300, layout='constrained')
        bot_gs = GridSpec(1, n_cols, figure=fig, width_ratios=gs_width_ratios, wspace=wspace)
    ...
    if show_top_row:
        ...
        if legend_entries is not None:
            ...
            # When legend_ncols=2 and there are 3 source columns, merge cols 1+ into right.
            if legend_ncols == 2 and len(legend_entries) > 2:
                _eff_entries = [legend_entries[0],
                                [e for col in legend_entries[1:] for e in col]]
            else:
                _eff_entries = legend_entries
            _n_leg_cols = len(_eff_entries)
            for col_idx, col_entries in enumerate(_eff_entries):
                handles = []
    ...
    # CL design line — drawn above gridlines but under all data (zorder=1.5)
    if cl_design is not None:
        _cl_design_specs = [
            (ax1, 0.98, 'right', 'top'),     # subplot 1: lower right
            (ax2, 0.02, 'left',  'bottom'),  # subplot 2: upper left
            (ax3, 0.02, 'left',  'top'),     # subplot 3: lower left
            (ax4, 0.98, 'right', 'bottom'),  # subplot 4: upper right
        ]
        for _ax, _tx, _ha, _va in _cl_design_specs:
            _ax.axhline(cl_design, color='k', lw=0.8, ls='-', zorder=1.5)
            _yoff = 4 if _va == 'bottom' else -4
            _ax.annotate(r'$C_L$ Design',
    ...
    # Major tick spacing
    for _ax in [_a for _a in [ax0, ax1, ax2, ax3, ax4] if _a is not None]:
        _ax.yaxis.set_major_locator(MultipleLocator(0.2))
    ax2r.yaxis.set_major_locator(MultipleLocator(0.04))

    # Minor gridlines
    for _ax in [_a for _a in [ax0, ax1, ax2, ax2r, ax3, ax4] if _a is not None]:
        _ax.minorticks_on()
...
def compare_airfoils(afl_dict_input, reynolds_numbers, turb_cases, tools, figurePath, color_override=None, alpha_min=-5, alpha_max=20, alpha_step=0.25, cl_design=None, legend_ncols=None, reverse_plot_order=False):
    ...
    # Reverse plotting order while keeping legend order intact.
    # Used for rainbow plots where we want clean rendered on top but legend 0->1.
    if reverse_plot_order and len(afl_dict) > 0:
        _n_per = len(data_list) // len(afl_dict)
        def _rev_blocks(lst, k):
            return [e for blk in [lst[i*k:(i+1)*k] for i in range(len(lst)//k)][::-1] for e in blk]
