...
def core_fitness_function(x):
    ...
    K_lower = x['individual'][int(len(x['individual'])/2):-1]
    ...
    try:
        ...
        te_gap_constraint = te_gap_lookup[str(int(100*tau))]
        ...
        tegap_violation_weighting           = 1000*constraint_base_penalty
        ...
        if lift_margin_clean >=0:
            ...
            conpen += lift_margin_clean_weighting*abs(lift_margin_clean)*constraint_base_penalty
        ...
        conpen += delta_cl_from_roughness_weighting * (max([pecent_delta_cl_from_roughness_threshold, abs(percent_delta_cl_clean_to_rough_at_alpha_design)])-pecent_delta_cl_from_roughness_threshold)
        ...
        if tau == '36':
            ...
            if cl_at_10_degrees_rough <= target_cl_36:
                conpen += CL_36_weighting * abs(target_cl_36-cl_at_10_degrees_rough)
        ...
        # ----------------------
        # Add a TE gap constraint
        # ----------------------
        if te_gap <= te_gap_constraint:
            conpen += tegap_violation_weighting * abs(te_gap_constraint - te_gap)
            cons.append(abs(te_gap_constraint-te_gap))
        else:
            cons.append(0.0)
...
if __name__ == '__main__':
    ...
    for i,ind in enumerate(airfoil_seeds):
        ...
        afl_geo.scaleThickness(0.21)
        ind = afl_geo.upperCoefficients.tolist() + afl_geo.lowerCoefficients.tolist() + [te_gap_lookup[str(int(t))]]
        # plt.figure()
